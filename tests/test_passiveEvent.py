...
class Test_PassiveEvent(TestCase):
    ...
    def test_noColide(self):
        ...
        self.status.HitBox = Rect(128 - 8, 256 - 16 - 24, 16, 24)
        """辺が比較対象の辺と接する場合は衝突判定にはならない"""
        terrObjs:list[Rect] = []
        for x in range(1, 17):
            terrObjs.append(Rect(x * 16, 256 - 16, 16, 16))
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.InAir, self.status.ReactionState)
        self.assertEqual(Rect(120, 216, 16, 24), self.status.HitBox)
    ...
    def test_existTopColide(self):
        ...
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.InAir, self.status.ReactionState)
        self.assertEqual(Rect(120, 224, 16, 24), self.status.HitBox)
    ...
    def test_existRightColide(self):
        ...
        self.status.HitBox.move_ip(1, 0)
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.InAir, self.status.ReactionState)
        self.assertEqual(Rect(112, 216, 16, 24), self.status.HitBox)
    ...
    def test_existRightColideOnLand(self):
        ...
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.Land, self.status.ReactionState)
        self.assertEqual(Rect(112, 217, 16, 24), self.status.HitBox)
    ...
    def test_existLeftColideOnLand(self):
        ...
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.Land, self.status.ReactionState)
        self.assertEqual(Rect(128, 217, 16, 24), self.status.HitBox)
    ...
    def test_fallRight(self):
        ...
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.InAir, self.status.ReactionState)
        self.assertEqual(Rect(121, 217, 16, 24), self.status.HitBox)
    ...
    def test_fallLeft(self):
        ...
        self.context.ApplyCollide(self.status, terrObjs)
        self.assertEqual(passiveEvent.ReactionState.InAir, self.status.ReactionState)
        self.assertEqual(Rect(119, 217, 16, 24), self.status.HitBox)